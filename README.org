...
These summary statistics are tabulated for all parameter combinations in file:modeling/simulation_runs/endocleave_rate_vary/sim.params.tsv  using the script [[file:modeling/combine_lifetime_and_psr_data.R]] which generates the =tsv= files in [[file:modeling/simulation_runs/endocleave_rate_vary/tables/]].
